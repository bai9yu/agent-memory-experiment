...
配对显著性检验显示，`supervised_set_selector` 相比 `type_aware` 的 MRR delta 为 `-0.0456`，95% CI `[-0.0893, -0.0017]`，permutation p-value `0.0366`；Recall@5 delta 为 `-0.0635`，95% CI `[-0.1349, 0.0079]`，p-value `0.1410`。因此当前 greedy set selector 不能作为有效改进，只能作为负结果和后续方法边界。

补充参数检查显示，`redundancy_weight=0.0` 是当前三组候选参数中相对最好的设置；`0.02` 和 `-0.02` 均没有改善 Coverage@5。
