...
| reproducibility | 当前仓库已经具备可复现的缓存实验包。 | Reproducibility artifact gate 196/196 and metric gate 22/22. | artifact_checked | 可以用于论文 appendix 和内部复现实验。 | 全新 clone 仍需要按文档准备模型/embedding cache，因为大缓存不进入 Git。 |
