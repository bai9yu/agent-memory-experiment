...
| reproducibility | 当前仓库已经具备可复现的缓存实验包。 | Reproducibility artifact gate 24/24 and metric gate 5/5. | artifact_checked | 可以用于论文 appendix 和内部复现实验。 | 全新 clone 仍需要按文档准备模型/embedding cache，因为大缓存不进入 Git。 |
...
| stability_protocol | DeepSeek memory writer 稳定性分析框架已经准备好，但重复抽取尚未完成。 | 稳定性 manifest 登记 3 次抽取，目前 completed runs=1；少于 3 次，不能报告方差。 | protocol_ready_pending_runs | 可以作为重复抽取实验入口；在 run2/run3 完成前，不应宣称 memory writer 稳定。 | 需要补齐至少 2 次 DeepSeek 重复抽取，并重新生成均值/标准差。 |
...
| open_gap | 完整项目距离最终投稿仍需要额外验证。 | 剩余缺口包括完成多 seed DeepSeek 抽取、实际完成更强 embedding/API baseline、更大真实 memory bank 效率实验，以及人工错误复核标注结果。 | gap_analysis | 作为下一步 checklist，而不是论文主张。 | 投稿前至少补齐一个强 baseline 家族，以及一个稳定性/可靠性检查。 |
