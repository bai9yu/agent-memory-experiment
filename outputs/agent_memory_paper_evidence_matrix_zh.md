...
| main_result | 2 |
| negative_result | 3 |
| negative_setwise_diagnostic | 1 |
| open_gap | 1 |
| positive_diagnostic | 1 |
...
| main_result | 事实级记忆相比 LoCoMo observation memory 能减少存储 token。 | Fact memory tokens 31148 vs observation 40241; ratio 0.774, saving about 22.6%. One-time writer API tokens are reported separately: 559574; token-only storage break-even diagnostic is 61.539 retrieval passes. | strong_cached | 可以支撑 memory compression / storage efficiency 动机。 | 若要报告货币成本，需要按目标 provider 的实时 input/output 单价换算；token-only break-even 不应写成费用或能耗结论。 |
...
| negative_result | 浅层 Type 3 修复方法无法解决多证据检索。 | Supervised set selector Coverage@5 delta -0.0572, p=0.0286; Type3-specific reranker and keyword decomposition also reduce Coverage@5. Top-20 coverage-aware unsupervised reranking also reduces Coverage@5 by -0.0396 and Full@5 by -0.0397. | statistically_supported_negative | 适合作为边界/负结果消融，而不是作为改进方法。 | 无监督 coverage/diversity 信号已不足；下一步应尝试 LLM 子问题生成或真正的 listwise/setwise objective。 |
| limited_positive | 保守的 Type 3 意图窗口重排可以微调首个证据位置，但不足以解决多证据覆盖。 | Intent-facet Top-5 window reranking changes MRR by +0.0013 while keeping Recall@5, Coverage@5, Full@5, Coverage@20, and Full@20 unchanged (+0.0000, +0.0000, +0.0000). | small_clean_diagnostic | 可作为轻量优化尝试或消融边界：保守窗口重排不会破坏 Top-5 证据集合，但收益很小。 | 需要学习式 set/listwise 目标来真正提升 Coverage@5 和 Full@5。 |
| diagnostic | Type 3 错误同时包含重排可救回空间和候选召回缺失。 | Within candidate-reranker Top-20, 39.7% of Type3 query-splits are rerank-rescuable; 25.4% miss all gold evidence. Oracle Top-5 coverage can rise from 0.372 to 0.597, with Full@5 rising from 0.262 to 0.444. | strong_diagnostic | 用于解释为什么下一步需要双线优化：listwise/setwise 重排负责可救回样本，召回增强负责候选缺失样本。 | 该结果是 oracle 上限分析，不能作为实际方法指标；实际算法仍需在 held-out query 上验证。 |
...
| negative_setwise_diagnostic | 无监督簇多样性不能直接替代答案证据覆盖目标。 | Cluster coverage selection changes Top-5 cluster count by +0.0635, but changes Coverage@5 by -0.0290 and Full@5 by -0.0556. The oracle gap on the same pool remains Coverage@5 +0.3455 and Full@5 +0.3016. | negative_setwise_diagnostic_with_oracle_gap | 可作为消融结论：集合级目标是必要的，但简单无监督多样性与 gold evidence coverage 不对齐。 | 需要用监督式 listwise/setwise 目标、子问题覆盖标签，或 LLM 生成的 query decomposition 来对齐证据覆盖。 |
...
| reproducibility | 当前仓库已经具备可复现的缓存实验包。 | Reproducibility artifact gate 241/241 and metric gate 22/22. | artifact_checked | 可以用于论文 appendix 和内部复现实验。 | 全新 clone 仍需要按文档准备模型/embedding cache，因为大缓存不进入 Git。 |
