...
| reproducibility | 当前仓库已经具备可复现的缓存实验包。 | Reproducibility artifact gate 84/84 and metric gate 5/5. | artifact_checked | 可以用于论文 appendix 和内部复现实验。 | 全新 clone 仍需要按文档准备模型/embedding cache，因为大缓存不进入 Git。 |
...
| baseline_protocol | 外部 embedding baseline 已经具备 API 接入与缓存框架，但尚未形成实验结果。 | 已登记 1 个外部 embedding baseline；completed=0, ready_or_completed=0；preflight required=4/5；预计文本 4355 条、约 71882 tokens、未缓存批次 35；对比表完成=False。 | protocol_ready_pending_run | 可以作为复现实验入口；在 summary.csv 生成前，不能写入主结果表。 | 需要提供 API key 并实际运行 text-embedding-3-small 等外部 embedding 对照。 |
