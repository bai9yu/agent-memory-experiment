...
| 2 | external_embedding | External embedding paper-ready baseline | blocker | completed external embedding baselines=0, postrun_pass=0; embedding_tier=pending | 运行真实 API embedding baseline、comparison 和 postrun gate。 |
...
| 5 | paper_claims | Manuscript claim and numeric consistency | pass | 8/8 claim checks pass; 15/15 numeric claim checks pass; scope_failures=0; numeric_failures=0 | 补完 blocker 后重新生成 manuscript、scope audit 和 numeric claim audit。 |
| 6 | reproducibility | Reproducibility, integrity, and freshness | pass | 200/200 artifacts exist; 22/22 metric thresholds pass; integrity manifest covers=200/200, sha256_ok=198, self_skips=2; stale_findings=0 | 任何实验或 artifact 变化后重跑 refresh、reproducibility、integrity 和 freshness。 |
...
| 8 | release | Public release and anonymization hygiene | pass | public release blockers=0; public_blockers=0 | 正式匿名投稿前按会议要求移除作者、仓库 URL、账号身份信息；开源前补 LICENSE。 |
