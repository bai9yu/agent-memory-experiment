...
| type3_coverage_aware_reranker | pass | 0 | 0.763 | Tests unsupervised coverage-aware Type3 reranking over cached Top-20 candidates. |
| type3_intent_fusion_reranker | pass | 0 | 6.366 | Tests conservative Type3 intent-facet window reranking without changing Top-5 evidence membership. |
...
| type3_supervised_window_reranker | pass | 0 | 1.022 | Tests dependency-free supervised Type3 window reranking with held-out query splits. |
| type3_recall_expansion_analysis | pass | 0 | 3.561 | Tests offline Type3 recall expansion by merging candidate Top-20 with multi-signal and intent-facet retrieval pools. |
| type3_expanded_pool_selector | pass | 0 | 4.332 | Tests whether expanded Type3 candidate pools can be converted into Top-5 evidence selection gains. |
...
| writer_cost_boundary | pass | 0 | 0.060 | Separates one-time LLM memory-write API tokens from reusable retrieval-time storage tokens. |
...
| paper_table_consistency | pass | 0 | 0.059 | Checks that paper Markdown/LaTeX tables are byte-identical to regenerated CSV-derived outputs. |
| untracked_artifact_audit | pass | 0 | 0.042 | Classifies untracked local outputs before public artifact packaging. |
...
| artifact_path_portability | pass | 0 | 0.238 | Checks tracked paper-facing reports for machine-local absolute paths before artifact sharing. |
| public_release_readiness | pass | 0 | 0.482 | Refreshes tracked-file release hygiene checks after untracked artifact audit. |
...
| submission_readiness | pass | 0 | 0.028 | Refreshes final-submission gates. |
...
| artifact_integrity_manifest_final | pass | 0 | 0.079 | Final manifest refresh after freshness audit changes. |
