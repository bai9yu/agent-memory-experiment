...
| embedding_baseline_status | pass | 0 | 0.022 | Refreshes external embedding key/result status without printing keys. |
...
| api_embedding_execution_runbook | pass | 0 | 0.023 | Generates the external API embedding baseline runbook without starting paid/network calls. |
| embedding_baseline_comparison | pass | 0 | 0.021 | Refreshes BGE-M3 vs API embedding comparison status from local summaries. |
| api_embedding_postrun_gate | pass | 0 | 0.022 | Checks whether any API embedding run has complete paper-ready local outputs. |
...
| submission_closure_consistency | pass | 0 | 0.022 | Checks closure plan, final checklist, readiness, reviewer prep, and strict API acceptance for consistent blocker standards. |
| submission_package_index | pass | 0 | 0.022 | Refreshes the index of manuscript, tables, appendices, gates, and packaging actions. |
| supplementary_package_manifest | pass | 0 | 0.028 | Builds a supplement packaging manifest with blocker and anonymization checks. |
| anonymous_submission_readiness | pass | 0 | 0.023 | Checks anonymous-submission readiness for current supplement candidates. |
| paper_table_consistency | pass | 0 | 0.051 | Checks that paper Markdown/LaTeX tables are byte-identical to regenerated CSV-derived outputs. |
| untracked_artifact_audit | pass | 0 | 0.034 | Classifies untracked local outputs before public artifact packaging. |
| large_intermediate_provenance | pass | 0 | 0.139 | Audits large local ranked/per-query intermediates against README commands and tracked downstream summaries. |
| artifact_path_portability | pass | 0 | 0.180 | Checks tracked paper-facing reports for machine-local absolute paths before artifact sharing. |
| public_release_readiness | pass | 0 | 0.430 | Refreshes tracked-file release hygiene checks after untracked artifact audit. |
| reproducibility_checklist | pass | 0 | 0.046 | Refreshes artifact and metric gates. |
| artifact_integrity_manifest | pass | 0 | 0.044 | Refreshes artifact sha256/size/line-count manifest. |
...
| final_submission_checklist | pass | 0 | 0.022 | Refreshes the action-oriented final-submission checklist. |
...
| submission_entrypoint_consistency | pass | 0 | 0.023 | Checks that README/package/reproducibility entrypoints point to the current submission readiness artifact. |
| paper_refresh_coverage | pass | 0 | 0.021 | Checks that the offline refresh run covers all required paper-facing reports. |
| artifact_integrity_manifest_final | pass | 0 | 0.043 | Final manifest refresh after freshness audit changes. |
