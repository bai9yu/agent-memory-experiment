...
| human_audit_paper_claim_upgrade | pass | 0 | 0.022 | Checks which paper-facing human-audit claim tier is currently unlocked. |
...
| api_embedding_preflight | pass | 0 | 0.037 | Refreshes paid/API embedding preflight without network calls. |
...
| submission_package_index | pass | 0 | 0.027 | Refreshes the index of manuscript, tables, appendices, gates, and packaging actions. |
...
| paper_table_consistency | pass | 0 | 0.053 | Checks that paper Markdown/LaTeX tables are byte-identical to regenerated CSV-derived outputs. |
| untracked_artifact_audit | pass | 0 | 0.035 | Classifies untracked local outputs before public artifact packaging. |
| large_intermediate_provenance | pass | 0 | 0.133 | Audits large local ranked/per-query intermediates against README commands and tracked downstream summaries. |
| artifact_path_portability | pass | 0 | 0.223 | Checks tracked paper-facing reports for machine-local absolute paths before artifact sharing. |
| public_release_readiness | pass | 0 | 0.475 | Refreshes tracked-file release hygiene checks after untracked artifact audit. |
...
| manuscript_claim_check | pass | 0 | 0.024 | Checks that manuscript does not overclaim pending baselines/audits. |
| manuscript_numeric_claim_check | pass | 0 | 0.032 | Checks that key numeric manuscript claims match current paper artifacts. |
| paper_scope_claim_audit | pass | 0 | 0.026 | Audits paper-facing documents for scope and generalization overclaims. |
| evidence_freshness | pass | 0 | 0.025 | Checks stale artifact/metric/integrity gate counts. |
...
| artifact_integrity_manifest_final | pass | 0 | 0.045 | Final manifest refresh after freshness audit changes. |
