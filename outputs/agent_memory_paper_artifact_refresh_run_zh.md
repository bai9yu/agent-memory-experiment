...
| type3_coverage_aware_reranker | pass | 0 | 0.767 | Tests unsupervised coverage-aware Type3 reranking over cached Top-20 candidates. |
...
| writer_cost_boundary | pass | 0 | 0.053 | Separates one-time LLM memory-write API tokens from reusable retrieval-time storage tokens. |
...
| submission_package_index | pass | 0 | 0.024 | Refreshes the index of manuscript, tables, appendices, gates, and packaging actions. |
...
| large_intermediate_provenance | pass | 0 | 0.141 | Audits large local ranked/per-query intermediates against README commands and tracked downstream summaries. |
| artifact_path_portability | pass | 0 | 0.236 | Checks tracked paper-facing reports for machine-local absolute paths before artifact sharing. |
...
| artifact_integrity_manifest | pass | 0 | 0.050 | Refreshes artifact sha256/size/line-count manifest. |
| evidence_matrix | pass | 0 | 0.031 | Refreshes paper claim/evidence/gap matrix. |
| submission_gap_analysis | pass | 0 | 0.024 | Refreshes reviewer-facing risk matrix. |
