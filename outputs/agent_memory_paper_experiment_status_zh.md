...
- `outputs/agent_memory_submission_readiness_gate_zh.md`：最终投稿门禁汇总；当前 required gates 为 `4/9`，blockers 为 `5`，说明复现、声明检查和公开发布卫生已通过，但外部 embedding baseline 和人工复核仍未达到最终投稿条件。
...
- `outputs/agent_memory_reproducibility_checklist_zh.md`：检查关键 artifact、核心指标阈值、数据规模和复现命令入口；当前 artifact gate 为 `61/61`，metric gate 为 `5/5`。
