...
进一步做 feature-group ablation 后，`intrinsic_only` candidate reranker 取得当前 held-out 最好结果：MRR `0.672`、Recall@5 `0.801`。它相比 fixed `type_aware` 的 MRR delta 为 `+0.0652`，95% bootstrap CI `[0.0543, 0.0759]`；相比原 full reranker 的 MRR delta 为 `+0.0113`，95% CI `[0.0032, 0.0199]`。这说明原 full reranker 中 method-level rank/score 特征可能带来噪声，而 candidate 自身语义、时间、类型、persona 和交互特征更适合作为论文主方法的简化版本。

扩展的 20-seed 稳定性实验进一步支持该结论：`intrinsic_only` 在 20/20 个随机划分 seed 上 MRR 均高于 fixed `type_aware`，跨 seed 平均 MRR `0.675` vs `0.615`，平均 ΔMRR `+0.0602`，最小 ΔMRR `+0.0414`；Recall@5 平均 `0.803` vs `0.738`。因此当前主方法增益不只是单一划分的偶然结果。

paired outcome / effect-size 分析显示，`intrinsic_only` 相比 fixed `type_aware` 的 MRR improved/worsened/tied 为 `771/591/1398`，paired Cohen's dz 为 `0.2234`；Recall@5 improved/worsened/tied 为 `280/92/2388`。按 query type 看，Type 2/4/5 明显受益，但 Type 3 Recall@5 delta 为 `-0.0476`，进一步支持“总体主方法有效，但多证据 Type 3 仍未解决”的写作边界。
...
已将 Type 3 query decomposition fusion4、supervised set selector `rw=0.0` 和 `rw=-0.02` 的报告、summary、split summary、coverage、per-query 和 ranked top20 文件纳入复现 artifact。它们共同支持一个更稳健的负结果结论：简单关键词拆解和浅层 greedy set selection 都不能替代真正的 listwise/setwise 多证据目标。
...
- `outputs/agent_memory_intrinsic_reranker_method_appendix_zh.md`：intrinsic feature reranker 方法附录；明确候选池构造、特征边界、RandomForest 超参数、held-out/LOCO 验证协议、bootstrap CI 和可复现命令。
- `outputs/agent_memory_intrinsic_reranker_feature_groups.csv`：intrinsic reranker 特征组表；区分 semantic/keyword、temporal、persona/type、categorical、importance 与被排除的 method-level rank/score 特征。
...
- `outputs/agent_memory_artifact_integrity_manifest_zh.md`：artifact 完整性 manifest；当前覆盖 `176/176` 个复现 artifact，缺失 `0`；其中 `174` 个文件记录稳定 sha256，manifest 自身 CSV/报告 `2` 个自引用文件标记为 `self_referential_skip`。
...
- `outputs/agent_memory_external_embedding_blocker_audit_zh.md`：外部 embedding baseline blocker audit；把默认 OpenAI key、通用 OpenAI-compatible provider、preflight、真实 summary 和 comparison table 的阻塞状态拆成可执行检查项，当前 blocker 数为 `3`。
- `outputs/agent_memory_external_embedding_blocker_audit.csv`：外部 embedding blocker 结构化表；记录每个检查项的 status、evidence、required_action 和解除的门禁。
...
- `outputs/agent_memory_human_audit_annotation_codebook_zh.md`：人工复核标注准则；统一 priority20/full80 盲审表中 yes/partial/no、gold sufficiency、人工错误类型和双人标注流程的判定标准。
- `outputs/agent_memory_human_audit_annotation_schema.csv`：人工复核字段 schema；记录每个 human_* 字段的问题、允许取值、判定规则和论文用途。

当前已生成论文复现清单：

- `outputs/agent_memory_reproducibility_checklist_zh.md`：检查关键 artifact、核心指标阈值、数据规模和复现命令入口；当前 artifact gate 为 `117/117`，metric gate 为 `9/9`。
...
6. 错误复核：已生成 80 条分层抽样复核表、DeepSeek LLM-assisted 预标注、Human/LLM 确认表、priority20 快速抽查包、盲审人工复核表以及 priority20/full80 阅读包；当前投稿风险矩阵将人工确认列为 blocker，建议先用 priority20 阅读包审阅 20 条样本并回填 blind review 的 `human_*` 字段，再用 full80 阅读包扩展到完整 80 条。
