...
def write_report(path: Path, root: Path) -> None:
    ...
    lines = [
        ...
        "## 6 错误分析与可靠性",
        "",
        f"当前已有 80 条 LLM-assisted audit 初稿，并生成 priority20 快速人工确认包。priority20 包包含 {priority_samples} 条样本，当前人工确认 {priority_confirmed} 条。该流程适合先低成本报告 quick-review agreement；完整投稿前仍应扩展到 80 条，并报告 exact agreement 与 Cohen's kappa。",
        "",
        "## 7 限制",
        "",
